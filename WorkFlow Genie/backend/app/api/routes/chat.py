...
class MessageRequest(BaseModel):
    ...
    sheet_name: Optional[str] = Field(None, description="Sheet name within the Excel file. If not provided or if the sheet doesn't exist, the first available sheet will be used automatically.", example="HR")
...
async def _handle_excel_operation(
    ...
    try:
        ...
        # ═══════════════════════════════════════════════════════════════
        # AUTO-DISCOVER SHEET NAME IF FILE_ID PROVIDED
        # ═══════════════════════════════════════════════════════════════
        ...
        if file_id:
            try:
                # Get file metadata to discover actual sheet names
                metadata_result = await mcp_service.execute_tool(
                    tool_name="get_file_metadata",
                    parameters={"file_id": file_id}
                )
                
                if metadata_result["success"]:
                    available_sheets = [s["name"] for s in metadata_result["data"]["sheets"]]
                    logger.info(f"📋 Available sheets in file: {available_sheets}")
                    
                    # If sheet_name not provided OR doesn't exist, use the first available sheet
                    if not sheet_name or sheet_name not in available_sheets:
                        actual_sheet_name = available_sheets[0] if available_sheets else "Sheet1"
                        if sheet_name:
                            logger.warning(f"⚠️ Sheet '{sheet_name}' not found. Using '{actual_sheet_name}' instead.")
                        else:
                            logger.info(f"📄 No sheet name provided. Auto-detected sheet: '{actual_sheet_name}'")
                    else:
                        actual_sheet_name = sheet_name
                        logger.info(f"✅ Using sheet: {actual_sheet_name}")
                        
            except Exception as e:
                logger.warning(f"Could not get file metadata: {e}")
                # Fallback to provided sheet_name or default
        ...
        if file_id:
            try:
                column_headers = await mcp_service.get_column_headers(file_id, actual_sheet_name)
                logger.info(f"📊 Column headers from '{actual_sheet_name}': {column_headers}")
            except Exception as e:
        ...
        plan = llm_service.plan_excel_operations(
            ...
            context={
                "file_id": file_id,
                "sheet_name": actual_sheet_name,  # Use discovered sheet name
                "available_sheets": available_sheets,  # Include all available sheets
