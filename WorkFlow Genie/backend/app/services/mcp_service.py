...
class MCPService:
    ...
    async def conditional_formatting(
        ...
        # Use the plain range string — MultiCellRange.to_tree() is broken in some openpyxl builds
    ...
    async def create_bubble_chart(
        ...
        chart.style = 18
        series = Series()
        series.xVal  = _ref(x_range)
        series.yVal  = _ref(y_range)
        series.bubbleSize = _ref(size_range)
    ...
    async def formula_conditional_formatting(
        ...
        rule = FormulaRule(formula=[formula_clean], dxf=dxf, stopIfTrue=False)
